...
console.log(naturalInputValidationPattern);
...
const DateTimeLocalInput = ({
  ...
  const formateSelectedDate = React.useCallback(
    (
      ...
      console.log(date, selectedDate, m, e);
      ...
      <PopoverContent className="w-auto p-0" sideOffset={8}>
        <div className="flex gap-1">
          <Calendar
            ...
            initialFocus
            disabled={(date) => {
              // Configurar la hora actual en la zona horaria de Perú
              const peruNow = new Date(
                new Date().toLocaleString("en-US", { timeZone: "America/Lima" })
              );
              const peruToday = startOfDay(peruNow); // Fecha de hoy sin horas/minutos

              // 1. Si la fecha es anterior a hoy
              if (date < peruToday || date < new Date("1900-01-01")) {
                return true; // Deshabilitar fechas anteriores
              }

              // 2. Si la fecha es hoy, deshabilitar las horas anteriores a la actual
              if (isToday(date)) {
                const currentHour = peruNow.getHours();
                const currentMinute = peruNow.getMinutes();
                const dateHour = date.getHours();
                const dateMinute = date.getMinutes();

                // Si la hora o minuto seleccionado es anterior a la hora actual
                if (
                  dateHour < currentHour ||
                  (dateHour === currentHour && dateMinute < currentMinute)
                ) {
                  return true;
                }
              }
